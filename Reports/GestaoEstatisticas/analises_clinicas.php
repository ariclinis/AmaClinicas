...
$Pegar_paci_atedindos=$pdo->prepare("SELECT distinct N_Consulta_Analise,DescAnalise , count(N_Consulta_Analise) as `quantidade` from `analises_marcadas_v` WHERE Estado_Fila='Atendido' && Estado_Consulta='Concluido' GROUP BY N_Consulta_Analise");
...
$pdf->Ln(13);
$pdf->Cell('180','8',utf8_decode('ESTATÍSTICAS POR ANALISES'),'','','C');
$pdf->Ln(5);
...
$pdf->SetFont('Arial', '', 9);
$cont2=0; while ($Linha = $Pegar_paci_atedindos->fetch(PDO::FETCH_ASSOC)) {
$cont2++;
        $pdf->Cell(15, $altura, $cont2, 1, 0, 'L');
        $pdf->Cell(75, $altura, utf8_decode($Linha['DescAnalise']), 1, 0, 'L');
